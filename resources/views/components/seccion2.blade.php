<div>
    <form id="formulario2" method="{{ strtoupper($method) === 'GET' ? 'GET' : 'POST' }}" action="{{ $action }}"
        enctype="multipart/form-data">
        @if (strtoupper($method) !== 'GET')
            @csrf
        @endif
        <input type="hidden" name="action" value="next"> <!-- Para diferenciar Siguiente vs Anterior -->
    ...
    @push('scripts')
        <script>
            function goToPreviousSection() {
                document.querySelector('#formulario2 input[name="action"]').value = 'previous';
            }

            // Set user roles for validations-direccion.js script
            window.userRoles = ['{{ Auth::user()->hasRole('solicitante') ? 'solicitante' : '' }}'];
        </script>
